...
const Header = ({ label, text }) => {
  return (
    <div className=" flex flex-col items-center justify-center">
      <p className="font-bold text-xs text-yellow-500 px-3 py-1 md:text-sm">{label.toUpperCase()}</p>
      <h3 className="my-5 mb-9 font-bold text-xl text-center text-slate-700 md:text-3xl">{text}</h3>
    </div>
  );
};
...
const ErrorMessage = ({ Error }) => {
  return (
    <div className="flex flex-col items-center justify-center">
      <p className="font-bold text-xs text-red-500 px-3 py-1 md:text-sm">{Error}</p>
    </div>
  );
};
  ...
  return (
    <article className="flex flex-col justify-between gap-1 md:gap-4  w-full md:max-w-40 p-2 border-2  rounded-2xl cursor-pointer transition ease-in-out hover:scale-105 transform-gpu duration-300 hover:border-yellow-300 hover:shadow-md">
      ...
      <div className="flex p-0 m-0 flex-col md:p-2 ">
        <h3 className="p-0  md:pt-3 font-bold w-full text-md md:text-3xl">{course.title}</h3>
        <p className="text-xs font-light text-left my-2 md:text-base">{course.description}</p>
      </div>
...
const CardCourse = ({ item }) => {
  console.log('---> item', item.default_price);
  //Tengo 2 componentes en el mismo componente
    ...
    <>
      <section className=" hidden md:flex bg-white  md:w-[95%] md:h-[400px] m-4 md:m-6 shadow-lg rounded-lg overflow-hidden justify-around p-2 ">
        <div className=" h-[100%] w-3/6 relative">
          <p className="absolute top-0  left-0  px-3 bg-blue-400 text-xs sm:text-sm text-white font-medium">{Status}</p>
          ...
          <div className="flex p-1 gap-5 flex-row  ">
            <Link href={`/course/${item.id}`}>
              <a className="w-full ]">
                <button className="w-full border-2 p-3 rounded-lg font-bold text-sm  border-primary text-primary   md:text-lg hover:bg-primary hover:text-white  transition duration-150 ease-out hover:ease-in">
                  Mas información
                </button>
              </a>
            </Link>
            ...
            <a className=" hidden sm:block sm:w-full">
              <button
                onClick={() => {
                  Payment(item.default_price);
                }}
                className=" w-full border-2 p-3 rounded-lg font-bold text-sm bg-primary text-white md:text-lg   hover:text-white hover:font-black hover:border-primary transition duration-150 ease-out hover:ease-in ">
                Comprar ahora
              </button>
            </a>
          </div>
      ...
      <section className=" my-4 p-2 md:hidden w-[90%] mx-auto h-[550px] flex flex-col justify-between bg-white rounded-2xl overflow-hidden shadow-2xl">
        <div className=" h-3/6 p-3">
          <div className="h-full relative grid place-content-center">
            <Image className="w-full" src={item.images[0]} alt={item.name} layout={'fill'} objectFit="contain" />
          </div>
        </div>
        <div className=" h-2/6">
...
export default function index({ Items, Error, label, text, type = 'bootcamp' }) {
  return (
    <section className="p-5 flex flex-col gap-6 ">
      <Header label={label} text={text} />
      {Error && <ErrorMessage Error={Error} />}
      {type == 'courses' && <CardCourseWrapper TopsCourses={Items}></CardCourseWrapper>}
      {Items && (
...
const CardCourseWrapper = ({ TopsCourses }) => {
  const Items = TopsCourses.slice(0, 5);
  ...
  return (
    <section className="m-0 p-0  w-full  max-w-[1200px] mx-auto relative ">
      <Link href={'/course'}>
        <p className="text-right py-2 md:px-10 px-5 font-semibold text-primary hover:underline">Ver todos los cursos</p>
      </Link>
      <div>
        <CardCourse item={CourseActive} />
      </div>
      <a
        onClick={prev}
      ...
      <div className="h-4 flex justify-center items-center space-x-5 ">
        {Items.map((item, index) => (
          <div
            key={'point' + index}
            className={`w-2 h-2 rounded-full cursor-pointer ${Step === index ? 'bg-primary' : ' bg-black/30'} `}></div>
        ))}
      </div>
    </section>
  );
